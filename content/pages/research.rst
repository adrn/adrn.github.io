apw/research
############

:url: research
:save_as: research.html

My research ranges from mapping dark matter in the Milky Way, to the formation and evolution of our Galaxy, to characterizing binary star systems. I've spent a lot of time thinking about current and future surveys of Milky Way stars and how to use these data to infer its dynamical history and structure. I'm particularly excited about the `Gaia mission <https://www.cosmos.esa.int/web/gaia>`_ and am a co-organizer of the `Gaia sprints <http://gaia.lol>`_. The pages below give a little more detail about my research, by category:

|

.. raw:: html

    <div class="text-center">
        <a href="research/dark-matter.html" class="imglink">
            <img src="pages/images/darkmatter.svg" width="15%" />
        </a>
        <span style="margin-left: 64px;"></span>
        <a href="research/milky-way.html" class="imglink">
            <img src="pages/images/galaxy.svg" width="15%" />
        </a>
        <span style="margin-left: 64px;"></span>
        <a href="research/binary-stars.html" class="imglink">
            <img src="pages/images/binary.svg" width="15%" />
        </a>
    </div>

|
|

Recent papers
=============

.. include:: ../extra/pubs.rst


Data products
=============

* `Binary companions of APOGEE DR16 stars </apogee-dr16.html>`_
* `Binary companions of evolved stars in APOGEE DR14 </twoface.html>`_
* `Gaia+Pan-STARRS data for the GD-1 stream <https://zenodo.org/record/1295543>`_
